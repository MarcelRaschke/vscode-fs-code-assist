import {
    ...
    IPCMessageReader, IPCMessageWriter, TextDocumentSyncKind
...
class ServiceDispatcher {
    ...
    private onDocumentSymbol(handler: DocumentSymbolParams): SymbolInformation[] {
        ...
        const analysis: Analysis.Analysis = this.perDocumentAnalysis.get(uri) as Analysis.Analysis;
    ...
    private async parseAndLintDocument(document: TextDocument) {
        ...
        const parseDocument = (): Promise<Diagnostic[]> => {
            return new Promise((resolve) => {
                try {
                    const analysis = new Analysis.Analysis();
                    analysis.end(documentText);
                    analysis.buildGlobalSymbols();
                    this.perDocumentAnalysis.set(documentUri, analysis);
                    return resolve([]);
                } catch (err) {
                    if (!(err instanceof SyntaxError)) { throw err; }
                    const e = err as any;
